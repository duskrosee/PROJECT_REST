...
// === ДОБАВЛЕННЫЕ ИМПОРТЫ ДЛЯ КУПОНОВ И ОПЛАТЫ (ШАГ 7) ===
...
apiRouter.post('/api/checkout/create', async (req, res) => {
    try {
        ...
        // Если передан купон, проверяем его перед созданием сессии
        if (couponCode) {
...
apiRouter.post('/api/checkout/:token/confirm', async (req, res) => {
    try {
        ...
        const { paymentMethod } = req.body; // например "VISA ****1234"

        // Вызываем сервис, передавая аргументы строго по порядку через запятую:
        const receipt = await transactionService.createTransaction(
            session.stationId,      // 1. stationId
            session.fuelId,         // 2. fuelId
            session.liters,         // 3. liters
            session.buyerName,      // 4. buyerName
            session.worker,         // 5. worker
            paymentMethod,          // 6. paymentMethod
            'opłacona',             // 7. status
            session.calcType,       // 8. calcType
            undefined,              // 9. amountPLN (оставляем пустым, так как считаем по литрам)
            session.couponCode,     // 10. couponCode
            session.token           // 11. checkoutToken
        );
