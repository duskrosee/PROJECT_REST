...
export default function App() {
  ...
  const handleCreateSale = async (e: React.FormEvent) => {
    ...
    const pricePerLiter = sfObj ? sfObj.pricePerLiter : 0;
    ...
    if (salesCalcType === 'liters') {
      ...
      finalLiters = Number(salesLiters);
      totalPreview = finalLiters * pricePerLiter;
    } else {
      if (Number(salesAmountPLN) <= 0) {
        setSalesError('Błąd: Kwota musi być większa od zera.');
        return;
      }
      totalPreview = Number(salesAmountPLN);
      finalLiters = totalPreview / pricePerLiter;
    }

    setDraft({
      stationId: salesStationId,
      fuelId: salesFuelId,
      liters: finalLiters,
      ...
      amountPLN: salesCalcType === 'price' ? totalPreview : undefined,
    });
    setPayOpen(true);
  };
    ...
    <div className="min-h-screen bg-[#0c0c0e] text-[#e1e1e3] selection:bg-white selection:text-black">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8">
        ...
        {activeTab === 'users' && (
          <div className="space-y-6">
            ...
            <div className="glass-card p-6 rounded-md flex justify-between items-center flex-wrap gap-4">
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-widest text-white">Zarządzanie użytkownikami</h3>
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#0c0c0e] border border-subtle rounded-sm p-8 w-full max-w-sm space-y-6 relative shadow-2xl">
            ...
            <div className="text-center pt-4 border-t border-subtle">
              ...
              <span className="text-[10px] font-mono text-zinc-400 block">user / user123</span>
            </div>
          </div>
        </div>
      )}
      ...
      {showUserModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#0c0c0e] border border-subtle rounded-sm p-8 w-full max-w-md space-y-6 shadow-2xl relative">
            <div className="flex justify-between items-center pb-4 border-b border-subtle">
              <h3 className="text-xs font-bold uppercase tracking-widest text-white">Rejestracja nowego użytkownika</h3>
      ...
      {draft && (
        <PaymentModal
          ...
          onPaid={(receipt: any) => {
            ...
            setSalesSuccess(
              hasDiscount
                ? `Sprzedaż zarejestrowana pomyślnie! Zniżka ${receipt.discountApplied} zł została zastosowana.`
                : 'Sprzedaż zarejestrowana pomyślnie!'
            );
