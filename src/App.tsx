...
export default function App() {
  ...
  // === ДОБАВЛЕНО ИЗ ШАГА 9: СТЕЙТЫ ДЛЯ МОДАЛКИ ОПЛАТЫ ===
  ...
  // === ИЗМЕНЕНО В ШАГЕ 9: ВМЕСТО ПРЯМОГО ЗАПРОСА ОТКРЫВАЕМ МОДАЛКУ ===
  const handleCreateSale = async (e: React.FormEvent) => {
    ...
    // Находим цену за литр, чтобы правильно посчитать превью
    ...
    // Сохраняем черновик и открываем модалку!
    setDraft({
  ...
  return (
      <div className="min-h-screen bg-[#0c0c0e] text-[#e1e1e3] selection:bg-white selection:text-black">
        ...
        {/* =============================================================
          MODAL: PAYMENT (DODANO Z KROKU 9)
          ============================================================= */}
        {draft && (
            <PaymentModal
                open={payOpen}
                ...
                onPaid={(receipt) => {
                  setPayOpen(false);
                  setSalesSuccess(`Sprzedaż zarejestrowana pomyślnie! Zniżka została zaaplikowana.`);
                  setSuccessNotice('Sprzedaż zrealizowana. Paliwo zostało pobrane ze stanu stacji!');
                  fetchData(); // odświeża listę i historię
                }}
            />
        )}
      </div>
