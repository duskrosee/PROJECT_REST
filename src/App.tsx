...
export default function App() {
  ...
  const handleSaveStation = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      lat: stationForm.lat !== '' ? parseFloat(stationForm.lat) : null,
      lng: stationForm.lng !== '' ? parseFloat(stationForm.lng) : null,
            ...
            <div className="glass-card p-6 rounded-md flex justify-between items-center flex-wrap gap-2">
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-widest text-white flex items-center gap-2">
                  <MapIcon className="w-3.5 h-3.5" /> Interaktywna Mapa Stacji Paliw
                </h3>
                <p className="text-[10px] text-zinc-500 mt-1">
                  Kliknij pin na mapie lub stację z listy po lewej, aby zobaczyć szczegóły. Możesz dodać współrzędne GPS podczas edycji stacji.
                </p>
              </div>
              <button
                onClick={fetchData}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-950 text-xs text-zinc-300 border border-subtle rounded-sm p-2.5 focus:border-white focus:outline-none placeholder-zinc-700 font-mono"
                  />
                </div>
